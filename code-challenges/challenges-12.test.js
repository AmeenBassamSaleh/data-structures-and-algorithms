...
const validatePin = (pin) => /^\d{4}$/g.test(pin); // should ask why


/* ------------------------------------------------------------------------------------------------
CHALLENGE 3

Write a function named validateWord that uses a regular expression pattern to validate that
 a word is between 5 and 10 characters long.
...
const validateWord = (word) => {
  ...
  let a = /\b([a-zA-Z]){5,10}\b/g; // need study
  return a.test(word);
};
...
const validateEmail = (email) => {
  // Solution code here...
  let a = /^[A-Za-z0-9]+\.?[A-Za-z0-9]+@[A-Za-z0-9]+.(net|com|org)$/;
  return a.test(email);
};
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  let a = /^(\((\d{3})\)[\s]?|(\d{3})[\s-]?)(\d{3})[\s-]?(\d{4})$/;
  return a.test(phoneNumber);
};
